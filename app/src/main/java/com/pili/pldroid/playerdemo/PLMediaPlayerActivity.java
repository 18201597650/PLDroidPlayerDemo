...
public class PLMediaPlayerActivity extends AppCompatActivity {
    ...
    private PLMediaPlayer.OnVideoSizeChangedListener mOnVideoSizeChangedListener = new PLMediaPlayer.OnVideoSizeChangedListener() {
        public void onVideoSizeChanged(PLMediaPlayer mp, int width, int height) {
            ...
            Log.d(TAG, "onVideoSizeChanged2222" + wid + " height:" + hig);
            // 根据视频的属性调整其显示的模式
            if (wid > hig) {
                if (getRequestedOrientation() != ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE) {
                    Log.d(TAG, "onVideoSizeChanged33333" );
                    setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);
                }
            } else {
                if (getRequestedOrientation() != ActivityInfo.SCREEN_ORIENTATION_PORTRAIT) {
                    Log.d(TAG, "onVideoSizeChanged55555" );
            ...
            Log.d(TAG, "onVideoSizeChanged66666"+ mSurfaceViewWidth+":::"+mSurfaceViewHeight);
            if (width > height) {
                // 竖屏录制的视频，调节其上下的空余
